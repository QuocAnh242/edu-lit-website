...
export default function LessonPage() {
  ...
  const handleCreateSession = (lessonId: string) => {
    navigate(`/lessons/${lessonId}/create-session`);
  };

  const handleViewSession = (lessonId: string, sessionId: string) => {
    navigate(`/lessons/${lessonId}/sessions/${sessionId}`);
  };
  ...
  const handleEditSession = (lessonId: string, sessionId: string) => {
    navigate(`/lessons/${lessonId}/sessions/${sessionId}/edit`);
  };
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-lg text-gray-600">
              {isTeacher()
                ? 'Manage lessons, sessions, and teaching materials'
                      ...
                      <AccordionTrigger className="w-full px-6 py-4 hover:no-underline [&[data-state=open]>div>svg]:rotate-90">
                        <div className="flex w-full items-start justify-between gap-4">
                          <div className="flex flex-1 items-start gap-4 text-left">
                            <FolderOpen className="mt-1 h-6 w-6 flex-shrink-0 transition-transform" />
                        ...
                        {isTeacher() && (
                          <div className="flex justify-end">
                            <Button
                              ...
                              variant="outline"
                              className="gap-2 border-slate-300 hover:border-slate-400 hover:bg-slate-50"
                              size="sm"
                            >
                              <Plus className="h-4 w-4" />
                              Create New Session
                        ...
                        {lesson.sessions.length === 0 ? (
                          ...
                          <div className="space-y-3">
                            {lesson.sessions.map((session, index) => (
                              <div
                                key={session.id}
                                className="group rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition-all hover:border-blue-300 hover:shadow-md"
                              >
                                <div className="flex items-start justify-between gap-4">
                                  <div className="flex-1">
                                    <div className="mb-2 flex items-center gap-2">
                                      ...
                                      <h4 className="font-semibold text-slate-800 transition-colors group-hover:text-blue-700">
                                        {session.title}
                                      </h4>
                                    </div>
                                  ...
                                  <div className="flex gap-1">
                                    <Button
                                      ...
                                      size="icon"
                                      onClick={() =>
                                        handleViewSession(lesson.id, session.id)
                                      }
                                      title="View Session"
            ...
            <Card className="border-blue-200 bg-blue-50">
              <CardContent className="p-6">
                <p className="text-sm text-blue-700">
                  💡 <strong>Note:</strong> You are viewing as a student. Click
                  on lesson titles to expand and view sessions. Use the{' '}
